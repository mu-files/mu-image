...
This document describes the rendering pipeline implemented in `muraw.dngio.DngPage.render`,
...
**Future work:** a general-purpose image compute graph (`muraw.engines`) is planned to
...
## muraw Implementation Status

Current implementation in `muraw.dngio.DngPage.render()`:

| Stage | muraw Status | Notes |
|-------|--------------|-------|
