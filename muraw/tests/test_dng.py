...
def generate_dng_structure_report(dng_file: DngFile, dng_file_path_for_naming: Path, output_dir: Path) -> None:
    ...
    with open(output_txt_path, 'w') as f_out: # dng_file is now passed as an argument
        ...
        try:
            ...
            f_out.write(f"Found {len(raw_pages_info)} raw page(s) with CFA or LinearRaw interpretation:\n")
            for idx, (page_id_val, photometric_name, shape_tuple) in enumerate(raw_pages_info):
                f_out.write(f"  - Raw Page (list index {idx}, page_id {page_id_val}): photometric='{photometric_name}', shape={shape_tuple}\n")
            ...
            for i, page in enumerate(tif.pages):
                ...
                subfiletype_str = page.subfiletype.name if page.subfiletype is not None else 'unknown'
                photometric_str = page.photometric.name if page.photometric is not None else 'unknown'
                ...
                for tag_id, tag_obj in page.tags.items():
                    ...
                    f_out.write(f"    {tag_name_str} ({tag_id}) [type: {value_type_str}]: {value_str}\n")

                if page.pages:
                    f_out.write(f"  -> Found nested 'pages' attribute with {len(page.pages)} sub-pages.\n")
                    for j, nested_page in enumerate(page.pages):
                        ...
                        nested_subfiletype_str = nested_page.subfiletype.name if nested_page.subfiletype is not None else 'unknown'
                        nested_photometric_str = nested_page.photometric.name if nested_page.photometric is not None else 'unknown'
                        nested_newsubfiletype_tag = nested_page.tags.get(254)
                        nested_dng_type_val = nested_newsubfiletype_tag.value if nested_newsubfiletype_tag is not None else -1
                        nested_dng_type_str = dng_subfiletype_map.get(nested_dng_type_val, 'unknown DNG type')
                        f_out.write(f"    -> Nested Page {j}: Shape={nested_array_str}, TIFF_type='{nested_subfiletype_str}', DNG_type='{nested_dng_type_str}', Photometric='{nested_photometric_str}'\n")
...
def _decode_and_save_cfa_page(raw_cfa_data: np.ndarray, cfa_pattern: str, output_jpeg_path: Path) -> None:
    ...
    print(f"        -> CFA Data: shape={raw_cfa_data.shape}, dtype={raw_cfa_data.dtype}, Pattern='{cfa_pattern}'")
    ...
    if debayer_code is None:
        ...
        if len(raw_cfa_data.shape) == 2:
            ...
            if scaled_gray_data is not None:
                ...
                print(f"          WARNING: Unsupported dtype {raw_cfa_data.dtype} for direct grayscale saving of unknown CFA pattern. Image not saved.")
    ...
    if color_image_full_depth.dtype == np.uint16:
        ...
        print(f"          WARNING: Debayered image has unexpected dtype {color_image_full_depth.dtype}. Attempting to normalize and scale.")
        try:
            ...
            print(f"          Could not convert debayered image of dtype {color_image_full_depth.dtype} to 8-bit.")
            return
...
def _decode_and_save_linear_page(raw_linear_data: np.ndarray, output_jpeg_path: Path) -> None:
    ...
    print(f"        -> LinearRaw Data: shape={raw_linear_data.shape}, dtype={raw_linear_data.dtype}")
    ...
    if processed_image_8bit is None:
        print(f"          Failed to process LinearRaw data for {output_jpeg_path.name}. Skipping save.")
        return
    ...
    elif not (len(processed_image_8bit.shape) == 2 or \
              (len(processed_image_8bit.shape) == 3 and processed_image_8bit.shape[2] == 1)):
        print(f"        -> LinearRaw data after scaling has unexpected shape {processed_image_8bit.shape}. Attempting to save as is.")
...
def decode_and_save_dng_images(dng_file: DngFile, dng_file_path_for_naming: Path, output_dir: Path) -> None:
    ...
    try:
        ...
        print(f"  Image Generation: Found {len(raw_pages_info)} raw page(s) with CFA or LinearRaw interpretation.")
        ...
        for page_id, photo_interp, shape in raw_pages_info:
            ...
            print(f"    Image Generation: Processing Raw Page ID {page_id} - photometric='{photo_interp}', shape={shape}, output='{output_jpeg_path.name}'")

            if photo_interp == 'CFA':
                print(f"      Attempting to get CFA data for page ID {page_id}...")
                raw_cfa_data, cfa_pattern = dng_file.get_raw_cfa_by_id(page_id)
                if raw_cfa_data is not None:
                    _decode_and_save_cfa_page(raw_cfa_data, cfa_pattern, output_jpeg_path)
                else:
                    # cfa_pattern contains the error message if raw_cfa_data is None
                    error_msg = f": {cfa_pattern}" if isinstance(cfa_pattern, str) and cfa_pattern else ""
                    print(f"        -> Failed to get CFA data for page ID {page_id}{error_msg}")
                ...
                if raw_linear_data is not None:
                    ...
                    print(f"        -> Failed to get LinearRaw data for page ID {page_id}")
            else:
                print(f"      Image Generation: Skipping page with unhandled photometric interpretation: {photo_interp}")
    except Exception as e:
        print(f"  Error during image decoding/saving for {dng_file_path_for_naming.name} (within decode_and_save_dng_images): {e}")
        import traceback
