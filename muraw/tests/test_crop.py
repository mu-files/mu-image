...
from muraw.tiff_metadata import Orientation
...
def test_view_rotate_crop_maps_canvas():
    ...
    t = mi.orientation(t, orientation=Orientation.ROTATE_90_CW)
...
def test_reverse_slice_matches_numpy(src, key, method):
    ...
    if method == "view":
        ...
        got = t[key]
    np.testing.assert_array_equal(got.realize(), src[key])
...
def test_ellipsis_fills_remaining_axes():
    ...
    np.testing.assert_array_equal(
        tr.view(np.s_[1:4, 2:6, ...]).realize(), rgb[1:4, 2:6]
    )
    np.testing.assert_array_equal(
        tr.view(np.s_[1:4, 2:6, ..., :]).realize(), rgb[1:4, 2:6]
    )
