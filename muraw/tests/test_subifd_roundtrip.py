...
1. Each raw SubIFD can be decoded by muraw's render_dng
...
from muraw.dngio import DngFile, IfdPageSpec, write_dng, SubFileType
...
def test_subifd_roundtrip(dng_path: Path, output_dir: Path):
    ...
    - {stem}_ifd{n}_muraw.tif - muraw render_dng output
    ...
    with DngFile(dng_path) as dng:
        ...
        for i, page in enumerate(pages):
            ...
            muraw_tif = output_dir / f"{stem}_ifd{i}_muraw.tif"
            dngvalidate_base = output_dir / f"{stem}_ifd{i}_dngvalidate"
            
            print(f"\n  Processing IFD {i} ({ifd_type}):")
            
            # 1. muraw render_dng -> {stem}_ifd{n}_muraw.tif
            try:
                ...
                tifffile.imwrite(str(muraw_tif), decoded)
                print(f"    -> {muraw_tif.name} ({decoded.shape})")
            ...
            # 3. Try muraw decode on the roundtrip DNG
            roundtrip_muraw_tif = output_dir / f"{stem}_ifd{i}_roundtrip_muraw.tif"
            ...
            try:
                ...
                if roundtrip_decoded is not None:
                    tifffile.imwrite(str(roundtrip_muraw_tif), roundtrip_decoded)
                    print(f"    -> {roundtrip_muraw_tif.name} ({roundtrip_decoded.shape})")
                else:
                    pytest.fail(f"muraw roundtrip decode returned None for IFD {i}")
            except Exception as e:
                pytest.fail(f"muraw roundtrip decode failed for IFD {i}: {e}")
            
            # 3a. Compare original muraw decode vs roundtrip muraw decode
            if roundtrip_decoded is not None and decoded.shape == roundtrip_decoded.shape:
                ...
                print(f"    original vs roundtrip (muraw): mean={stats['mean']:.4f}%, max={stats['max']:.4f}%")
                # Fail if roundtrip through muraw produces significantly different results
                if stats['mean'] > 1.0:
                    pytest.fail(
                        f"Roundtrip through muraw produced different results: "
            ...
            # 5. Compare original muraw vs dng_validate (both decoding roundtrip DNG)
            if decoded.shape == dngvalidate_out.shape:
                stats = compute_diff_stats(decoded, dngvalidate_out)
                print(f"    muraw vs dng_validate: mean={stats['mean']:.4f}%, max={stats['max']:.4f}%")
                # Fail if muraw and dng_validate produce very different results
                if stats['mean'] > 1.0:
                    pytest.fail(
                        f"muraw and dng_validate produced very different results: "
                        f"mean={stats['mean']:.2f}%, max={stats['max']:.2f}%"
                    )
            else:
                print(f"    Shape mismatch: muraw {decoded.shape} vs dng_validate {dngvalidate_out.shape}")
