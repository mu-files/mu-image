...
"""Command-line interface for muraw."""
...
def cli(verbose, timing):
    """muraw - Image processing utilities."""
...
def dng_raw_stage(input_file, output_file, stage, ifd, demosaic):
    ...
    Examples:
      ...
      muraw dng raw-stage input.dng output.tif camera-rgb --ifd subifd2
      muraw dng raw-stage input.dng output.tif raw --demosaic EA
    """
    import numpy as np
    from .dngio import DngFile
    from . import raw_render
...
def dng_copy(
    ...
    timer = PerfTimer("dng_copy", start_time=muraw._app_start_time)
...
def batch_convert_dngs(
    ...
    """
    import setproctitle
    setproctitle.setproctitle("muraw: batch-convert")
...
def convert_dngs_to_video(
    ...
    """
    import setproctitle
    setproctitle.setproctitle(f"muraw: encoding {Path(output_mp4).name}")
...
def google_photos():
    ...
    Requires: pip install muraw[all]
    """
...
def auth(credentials, token_path, force):
    ...
    try:
        ...
        if client.authenticate(force_reauth=force):
            ...
            click.echo("\nYou can now use 'muraw google-photos upload' for automated uploads")
            sys.exit(0)
        else:
...
def upload(image_path, album, token_path):
    ...
    try:
        ...
        if not client.authenticate():
            click.echo(
                "✗ Authentication failed. Run 'muraw google-photos auth' first.",
                err=True,
            )
            sys.exit(1)
